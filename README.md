<!-- ================================================================
     GITHUB PROFILE README
     Username set to Chandradip17. Still replace YOUR_LINKEDIN,
     YOUR_EMAIL, and YOUR_PORTFOLIO below before publishing.
     ================================================================ -->

<div align="center">

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=26&duration=2800&pause=800&color=58A6FF&center=true&vCenter=true&width=560&lines=AI+Developer;Web+Developer;Problem+Solver;Hackathon+Builder;Computer+Science+Student" alt="Typing SVG" />

## Chandradip Saha

### Building intelligent software at the intersection of AI and the Web.

<sub>Computer Science & Technology student · AI + Web Development</sub>

</div>

<br/>

## 🧠 About Me

I'm a Computer Science & Technology student who likes building practical software more than collecting certificates. My focus areas right now are **AI** and **Web Development**, with a strong interest in problem solving, hackathons, and project-based learning.

I care about understanding *how* systems work — not just copying tutorials — and I'm continuously sharpening my development and problem-solving skills through real projects.

<br/>

## 🧰 Tech Stack

<table>
<tr>
<td valign="top" width="33%">

**Languages**
<br/>
<img src="https://img.shields.io/badge/C-00599C?style=flat-square&logo=c&logoColor=white" />
<img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white" />
<img src="https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=openjdk&logoColor=white" />
<img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black" />

</td>
<td valign="top" width="33%">

**Frontend**
<br/>
<img src="https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white" />
<img src="https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white" />

**Tools**
<br/>
<img src="https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white" />
<img src="https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github&logoColor=white" />
<img src="https://img.shields.io/badge/VS%20Code-007ACC?style=flat-square&logo=visualstudiocode&logoColor=white" />

</td>
<td valign="top" width="33%">

**Exploring**
<br/>
<img src="https://img.shields.io/badge/AI%2FML-grey?style=flat-square&logo=OpenAI&logoColor=white" />
<img src="https://img.shields.io/badge/APIs-grey?style=flat-square&logo=fastapi&logoColor=white" />
<img src="https://img.shields.io/badge/Databases-grey?style=flat-square&logo=postgresql&logoColor=white" />

</td>
</tr>
</table>

<br/>

## 🧪 Build Lab

My repositories are where I build, experiment, learn, and improve. This is a rough roadmap of what I'm working on or planning.

| Project | Focus | Status |
|---|---|---|
| 🤖 **AI StudyMate** — AI-powered study companion | AI · Education · Web Dev | Planned |
| 🎵 **VibeSage** — Mood-based music recommendation concept | NLP · Recommendation Systems · APIs | Planned |
| ♻️ **Smart Waste Sorter** — Arduino-based smart waste sorting | Arduino · Sensors · Automation | Planned |
| ☕ **Java Projects** — Java/OOP learning & applications | Java · OOP | In progress |
| 🧠 **DSA Lab** — Data structures & algorithm practice | DSA | Ongoing |

<br/>

## 🌟 Featured Projects

<table>
<tr>
<td width="50%" valign="top">

### 🤖 AI StudyMate
An AI-powered study companion concept.

- **Tech:** Python, AI/NLP tools, Web
- **Problem:** Studying efficiently is hard without structured guidance
- **Solution:** An assistant that helps organize and reinforce learning
- **Status:** `Planned`
- 🔗 [Repository](https://github.com/Chandradip17/ai-studymate) · 🌐 [Demo](#)

</td>
<td width="50%" valign="top">

### 🎵 VibeSage
A mood-based music recommendation concept.

- **Tech:** Python, NLP, Recommendation Systems, APIs
- **Problem:** Music discovery rarely accounts for mood
- **Solution:** Recommend tracks based on inferred mood
- **Status:** `Planned`
- 🔗 [Repository](https://github.com/Chandradip17/vibesage) · 🌐 [Demo](#)

</td>
</tr>
<tr>
<td width="50%" valign="top">

### ♻️ Smart Waste Sorter
An Arduino-based smart waste sorting project.

- **Tech:** Arduino, Sensors, Automation
- **Problem:** Manual waste sorting is inconsistent
- **Solution:** Automated sensor-based sorting mechanism
- **Status:** `Planned`
- 🔗 [Repository](https://github.com/Chandradip17/smart-waste-sorter) · 🌐 [Demo](#)

</td>
<td width="50%" valign="top">

### 🧠 DSA Lab
Data structures and algorithm practice repository.

- **Tech:** C, Python, Java
- **Problem:** Strong fundamentals need consistent practice
- **Solution:** A structured, growing collection of DSA solutions
- **Status:** `Ongoing`
- 🔗 [Repository](https://github.com/Chandradip17/dsa-lab) · 🌐 [Demo](#)

</td>
</tr>
</table>

<br/>

## 📊 GitHub Stats

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=Chandradip17&show_icons=true&theme=default&hide_border=true&count_private=true" alt="GitHub Stats" height="165"/>
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Chandradip17&layout=compact&hide_border=true&theme=default" alt="Top Languages" height="165"/>

<br/>

<img src="https://streak-stats.demolab.com?user=Chandradip17&hide_border=true&theme=default" alt="GitHub Streak" />

</div>

<br/>

## 🐍 Contribution Snake

<div align="center">

<img src="https://raw.githubusercontent.com/Chandradip17/Chandradip17/output/github-contribution-grid-snake.svg" alt="Contribution Snake animation" />

</div>

> Generated automatically by a GitHub Action. See `.github/workflows/snake.yml` and the setup instructions below.

<br/>

## 📈 Currently Building

- 🤖 AI-powered applications
- 🌐 Web development
- 🧠 Data Structures & Algorithms
- 🔌 APIs
- 🗄️ Databases
- 🏆 Hackathon projects

<br/>

## 📚 Learning Roadmap

```
Programming
   ↓
Data Structures & Algorithms
   ↓
Web Development
   ↓
APIs + Databases
   ↓
AI / Machine Learning
   ↓
Full-Stack AI Applications
```

<br/>

## 🏆 Hackathon Mindset

```
Problem → Research → Prototype → Build → Test → Deploy → Improve
```

I care more about building solutions to real problems than about demonstrating technologies for their own sake.

<br/>

## 📌 Principles

| |
|---|
| **Build** > Talk |
| **Projects** > Certificates |
| **Consistency** > Hype |
| **Learn** > Copy |
| **Document** > Hide |

<br/>

## 📫 Contact

<div align="center">

[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Chandradip17)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](YOUR_LINKEDIN)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:YOUR_EMAIL)
[![Portfolio](https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white)](YOUR_PORTFOLIO)

</div>

<br/>

<div align="center">
<sub>Thanks for stopping by — feel free to explore the repositories above.</sub>
</div>
